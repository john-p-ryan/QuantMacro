# Two-asset household model: primitives and double-grid-search VFI solver.
# Definitions only (no top-level execution) so it can be included by
# grid_search.jl (driver script) and test_grid_search.jl.
