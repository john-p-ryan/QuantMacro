# This file contains code to solve the Aiyagari model using endogenous grid method (EGM) and policy iteration.
...
@with_kw mutable struct Results
    ...
    c_splines::Vector{PchipSplineInterpolation} # splines for consumption function
...
function initialize(;k_min=1e-6, k_max=60.0, nk=100, n_hist=125)
    ...
    c_splines = [PchipSpline(k_grid, c_policy[:, z_index]) for z_index in 1:nz]
...
function bellman(prim::Primitives, res::Results)
    ...
    for (z_index, z) in enumerate(z_grid)
        ...
        c_next[:, z_index] = evaluate_spline(c_spl_today, k_grid)
        # calculate capital policy function from budget constraint
        k_next[:, z_index] = (1+r-δ) * k_grid .+ w * ē * z - c_next[:, z_index]
        
        # check for binding borrowing constraint
        binding = k_next[:, z_index] .< k_min
        if any(binding)
            k_next[binding, z_index] .= k_min
            c_next[binding, z_index] .= (1+r-δ) * k_grid[binding] .+ w * ē * z .- k_min
        end
    end
...
function policy_iteration!(prim, res; tol=1e-8, max_iter=10_000)

    error = 100 * tol
    iter = 0

    while error > tol && iter < max_iter
        k_next, c_next = bellman(prim, res)
        error = maximum(abs.(c_next - res.c_policy))
        res.k_policy, res.c_policy = k_next, c_next
        iter += 1
    end

    if iter == max_iter
        println("Maximum iterations reached in policy iteration")
    elseif error < tol
        println("Converged in policy iteration after $iter iterations")
    end

    c_spl = [PchipSpline(prim.k_grid, res.c_policy[:, z_index]) for z_index in eachindex(prim.z_grid)]
    res.c_splines = c_spl
...
function steady_state_capital!(prim, res)
    ...
    function r_error(r)
        ...
        policy_iteration!(prim, res; tol=1e-9, max_iter=10_000)
        steady_dist!(prim, res; tol=1e-9, max_iter=10_000)
        capital_supply = sum(res.μ, dims=2) ⋅ k_hist
        return (capital_supply - res.K)^2
    end

    opt = optimize(r_error, δ, .08, abs_tol=1e-9)
    res.r = opt.minimizer
    res.K = L * (α / res.r) ^ (1 / (1-α))
    res.w = (1-α) * (res.K / L) ^ α
    # perform one last fine iteration to get steady state
    policy_iteration!(prim, res; tol=1e-12, max_iter=20_000)
...
function calculate_aggregates!(prim, res)
    @unpack_Primitives prim
    # This version is safer to avoid scope issues.

    # calculate aggregate capital supply
    K = sum(res.μ, dims=2) ⋅ prim.k_hist
    res.K = K # Update the struct

    # calculate aggregate consumption
    c_pol_hist = zeros(prim.n_hist, prim.nz)
    for z_index in eachindex(prim.z_grid)
        c_pol_hist[:, z_index] = evaluate_spline(res.c_splines[z_index], prim.k_hist)
    end
    C = res.μ ⋅ c_pol_hist
...
function recover_V!(prim::Primitives, res::Results; tol=1e-10, max_iter=20_000)
    # Recover V from fixed policies: V(k,z) = u(c_policy(k,z)) + β * E[V(k_policy(k,z), z')]
    @unpack_Primitives prim
    @unpack_Results res


    # Good initial guess: geometric series under fixed policies
    ...
    while err > tol && iter < max_iter
        # Build splines of current V for each z' to evaluate at off-grid k'
        V_spl = [PchipSpline(k_grid, V[:, z_next]) for z_next in 1:nz]

        V_new = similar(V)
        for z_idx in 1:nz
            kprime = clamp.(res.k_policy[:, z_idx], k_min, k_max)  # guard against tiny drift
            # Expected continuation value E[V(k', z')]
            EV = zeros(nk)
    ...
    if iter == max_iter
        println("recover_V!: reached max_iter with err = $err")
    else
        println("recover_V!: converged in $iter iterations (err = $err)")
    end
